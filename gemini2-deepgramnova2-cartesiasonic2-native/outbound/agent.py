"""Outbound voice agent — Gemini LLM + Deepgram STT + Cartesia TTS + call state management.
        ...
        try:
            async with self._session.post(url, headers=headers, json=payload) as resp:
                if resp.status == 200:
                    ...
                    logger.debug(
                        f"Cartesia TTS: synthesized {len(audio_data)} bytes"
                    )
                    return audio_data
                else:
