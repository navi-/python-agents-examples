...
Requirements:
    - Valid PLIVO_AUTH_ID, PLIVO_AUTH_TOKEN, PLIVO_PHONE_NUMBER, PLIVO_TEST_NUMBER,
      GEMINI_API_KEY, DEEPGRAM_API_KEY, CARTESIA_API_KEY in .env
    - ngrok binary available on PATH
    - Port 18003 available

Usage:
    cd gemini2-deepgramnova2-cartesiasonic2-native
...
pytestmark = pytest.mark.skipif(
    not all([
        ...
        PLIVO_TEST_NUMBER, GEMINI_API_KEY, DEEPGRAM_API_KEY, CARTESIA_API_KEY,
    ]),
    reason=(
        "Plivo credentials, PLIVO_TEST_NUMBER, or API keys not configured"
    ),
)
...
def bleg_app_id(plivo_client, ngrok_tunnel):
    ...
    if original_app and "/Application/" in str(original_app):
        original_app_id = (
            str(original_app).split("/Application/")[1].rstrip("/")
        )

    # Create or update the test app
    app_name = "Gemini_Deepgram_Cartesia_Outbound_Test"
    ...
    if original_app_id:
        plivo_client.numbers.update(
            number=test_digits, app_id=original_app_id
        )
        print(
            f"\n[Plivo] Restored {test_digits} to original app "
            f"{original_app_id}"
        )
...
class TestOutboundCall:
    ...
    def test_initiate_outbound_call_api(
        ...
        resp = httpx.post(
            ...
            params={
                ...
                "opening_reason": (
                    "your recent demo request for TechFlow Teams"
                ),
    ...
    def test_outbound_answer_webhook(
        ...
        assert "ws" in body.lower(), (
            "Stream should contain WebSocket URL"
        )
    ...
    def test_outbound_call_full_cycle(
        ...
        if not call_uuid:
            print(
                "[Outbound] Call did not go live — "
                "skipping recording verification"
            )
            pytest.skip(
                "Call did not connect (callee may not have answered)"
            )
        ...
        try:
            ...
            try:
                ...
                print(
                    f"[Outbound] Hangup error (may already be ended): {e}"
                )
        ...
        assert len(audio_data) > 1000, (
            f"Recording too small: {len(audio_data)} bytes"
        )
        ...
        assert len(transcript) > 5, (
            f"Transcript too short: '{transcript}'"
        )
