...
app = FastAPI(
    title="Gemini-Deepgram-Cartesia Voice Agent (Inbound)",
    description="Inbound voice agent using Gemini LLM, Deepgram STT, and Cartesia TTS",
    version="0.1.0",
)
...
async def health_check() -> dict:
    ...
    return {
        "status": "ok",
        "service": "gemini2-deepgramnova2-cartesiasonic2-voice-agent-inbound",
...
def main() -> None:
    ...
    logger.info(
        f"Starting Gemini-Deepgram-Cartesia Inbound Voice Agent on port {SERVER_PORT}"
    )
    ...
    uvicorn.run(
        "inbound.server:app", host="0.0.0.0", port=SERVER_PORT, log_level="info"
    )
