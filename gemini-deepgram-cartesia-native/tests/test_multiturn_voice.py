...
def generate_tts_audio(text: str) -> bytes | None:
    """Generate speech audio using Google TTS and convert to u-law."""
    try:
...
async def send_audio_and_wait(
    ws, audio_bytes: bytes, timeout: float = 15.0
) -> dict:
    ...
    # Send in 20ms chunks (160 bytes at 8kHz u-law)
    chunk_size = 160
    chunks = [
...
async def test_multiturn_voice():
    ...
    ws_url = (
        ...
        "eyJjYWxsX3V1aWQiOiAidGVzdCIsICJmcm9tIjogIisxNTU1MTIzN"
        "DU2NyIsICJ0byI6ICIrMTY1NzIzMzg4OTIifQ=="
    )
    ...
    for i, text in enumerate(turns):
        ...
        if audio is None:
            ...
            print(
                "  Make sure ffmpeg is in PATH and gTTS/pydub are installed"
            )
            return
    ...
    try:
        async with websockets.connect(ws_url, close_timeout=5) as ws:
            ...
            for i, (text, audio) in enumerate(turn_audio):
                ...
                if result["recv_chunks"] > 0:
                    ...
                    if result["ttfr"]:
                        print(
                            f"  Time to first response: {result['ttfr']:.1f}s"
                        )
                else:
            ...
            print("=" * 70)
            print(
                f"{'Turn':<5} {'Description':<32} "
                f"{'Sent':>10} {'Recv':>10} {'TTFR':>8}"
            )
            print("-" * 70)
            ...
            for i, (desc, sent, recv, ttfr) in enumerate(results):
                ...
                print(
                    f"{i:<5} {desc:<32} "
                    f"{sent:>9}B {recv:>9}B {ttfr_str:>8}"
                )
                total_recv += recv
        ...
        print(
            "Make sure the server is running: uv run python -m inbound.server"
        )
    except Exception as e:
        print(f"\nERROR: {e}")
        import traceback
