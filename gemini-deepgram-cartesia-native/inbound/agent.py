"""Inbound voice agent — Gemini LLM + Deepgram STT + Cartesia TTS engine.
...
class VoiceAgent:
    """Voice agent that orchestrates STT, LLM, TTS, and VAD."""
    ...
    async def _handle_transcript(self, transcript: str) -> None:
        ...
        async with self._processing_lock:
            ...
            ulaw_audio = cartesia_to_plivo(tts_audio)

            # Queue for sending
            await self._send_queue.put(ulaw_audio)
            self._is_responding = False
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            greeting = (
                "Hello! This is Alex, the Gemini native agent, "
                "powered by Deepgram and Cartesia. How can I help you today?"
            )
            greeting_audio = await self._tts.synthesize(greeting)
            if greeting_audio:
                ulaw_audio = cartesia_to_plivo(greeting_audio)
                await self._send_queue.put(ulaw_audio)
    ...
    async def _receive_from_plivo(self) -> None:
        """Receive audio from Plivo, run VAD, and send to STT."""
        try:
            while self._running:
                        ...
                        if speech_started and self._is_responding:
                            ...
                            while not self._send_queue.empty():
                                try:
                                    ...
                                    break
                            # Send clearAudio to stop playback
                            await self.websocket.send_text(
                                json.dumps({"event": "clearAudio"})
                            )

                        if speech_ended:
                            logger.debug("VAD: speech ended, resetting")
                            self._vad.reset()
    ...
    async def _send_to_plivo(self) -> None:
        ...
        PLIVO_CHUNK_SIZE = 160  # 20ms at 8kHz u-law
