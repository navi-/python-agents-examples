...
- Pipeline converters for Muse Voice Transcribe (STT) and Cartesia (TTS)
- Silero VAD processor for barge-in detection
"""
...
MUSE_SAMPLE_RATE = 24000  # Muse realtime `pcm16` input is 24kHz PCM16 (g711_ulaw path is 8kHz)
CARTESIA_SAMPLE_RATE = 24000  # Cartesia Sonic-3 output requested as 24kHz PCM16
...
# Start threshold is deliberately high: acoustic echo of the agent's own Cartesia
# playback coming back over the PSTN leg registers ~0.5-0.75 speech probability,
# while real caller speech registers 0.9+. 0.85 rejects echo without missing
# people talking over the agent (barge-in).
VAD_START_THRESHOLD = 0.85
...
def plivo_to_muse(mulaw_8k: bytes) -> bytes:
    """Convert Plivo audio (u-law 8kHz) to Muse `pcm16` input (PCM16 24kHz).

    Only used when MUSE_INPUT_FORMAT=pcm16. The default `g711_ulaw` path forwards
    Plivo's u-law bytes to Muse untouched (no decode, no resample).
    """
    pcm_8k = ulaw_to_pcm(mulaw_8k)
    return resample_audio(pcm_8k, PLIVO_SAMPLE_RATE, MUSE_SAMPLE_RATE)


def cartesia_to_plivo(pcm_24k: bytes) -> bytes:
    """Convert Cartesia audio (PCM16 24kHz) to Plivo format (u-law 8kHz)."""
...
def plivo_to_vad(mulaw_8k: bytes) -> np.ndarray:
    """Convert Plivo audio (u-law 8kHz) to float32 16kHz for Silero VAD."""
    pcm_8k = ulaw_to_pcm(mulaw_8k)
...
class SileroVADProcessor:
    ...
    Tracks speech state transitions (silence -> speaking -> silence). In this
    example the VAD is the barge-in trigger (speech start during playback) and a
    safety net for turn commits; Muse Voice Transcribe's built-in endpointing is
    the primary end-of-turn signal.
    """
