"""
End-to-end live tests for the Babel voice agent
(Muse Voice Transcribe + GPT-5.4 mini + Cartesia Sonic-3).
...
Requirements:
    - Valid OPENAI_API_KEY, META_API_KEY, CARTESIA_API_KEY in .env
...
pytestmark = pytest.mark.skipif(
    not all([OPENAI_API_KEY, META_API_KEY, CARTESIA_API_KEY]),
    reason="OPENAI_API_KEY, META_API_KEY, or CARTESIA_API_KEY not configured",
)
...
class TestE2ELive:
    ...
    async def test_agent_greeting(self, server_process):
        ...
        greeting_words = ["hello", "hi", "welcome", "babel", "bistro", "mundo", "table", "language"]
    ...
    async def test_agent_responds_to_text(self, server_process):
        ...
        async with websockets.connect(ws_url, close_timeout=3) as ws:
            ...
            await ws.send(
                json.dumps(
                    {
                        "event": "text",
                        "text": "What vegetarian dishes do you have and how much are they?",
                    }
                )
            )
        ...
        wav = pcm16_to_wav(pcm, sample_rate=8000)
        transcript = transcribe_audio_local(wav)
        print(f"[Menu response transcript]: {transcript}")
        ...
        assert len(matches) >= 2, (
            f"Response doesn't discuss the menu enough. "
