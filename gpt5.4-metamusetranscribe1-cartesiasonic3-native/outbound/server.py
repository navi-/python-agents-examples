...
app = FastAPI(
    title="Babel: Muse Voice Transcribe + GPT-5.4 mini + Cartesia Sonic-3 (Outbound)",
    description=(
        "Outbound concierge: Meta Muse Voice Transcribe STT + GPT-5.4 mini + Cartesia TTS + Plivo"
    ),
    version="0.1.0",
)
...
async def health_check() -> dict:
    ...
    return {
        "status": "ok",
        "service": "gpt5.4-metamusetranscribe1-cartesiasonic3-voice-agent-outbound",
...
def main() -> None:
    ...
    logger.info(
        "Starting Babel (Muse + GPT-5.4 mini + Cartesia) Outbound Voice Agent "
