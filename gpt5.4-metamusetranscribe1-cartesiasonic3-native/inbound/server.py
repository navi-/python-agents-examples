...
try:
    ...
    Traceloop.init(app_name="gpt5.4-metamusetranscribe1-cartesiasonic3-native")
...
app = FastAPI(
    title="Babel: Muse Voice Transcribe + GPT-5.4 mini + Cartesia Sonic-3 (Inbound)",
    description=(
        "Inbound speakerphone concierge: Meta Muse Voice Transcribe STT + "
        "GPT-5.4 mini + Cartesia TTS + Plivo"
    ),
    version="0.1.0",
)
...
def configure_plivo_webhooks() -> bool:
    ...
    try:
        ...
        app_name = "Babel_Muse_GPT_Cartesia_Voice_Agent"
...
async def health_check() -> dict:
    ...
    return {
        "status": "ok",
        "service": "gpt5.4-metamusetranscribe1-cartesiasonic3-voice-agent-inbound",
...
def main() -> None:
    ...
    logger.info(
        f"Starting Babel (Muse + GPT-5.4 mini + Cartesia) Inbound Voice Agent on port {SERVER_PORT}"
    )
