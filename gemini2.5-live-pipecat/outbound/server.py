...
app = FastAPI(
    title="Gemini Live Pipecat Voice Agent (Outbound)",
    description="Outbound voice agent using Pipecat with Google Gemini Live API",
    version="0.1.0",
)
...
async def health_check() -> dict:
    ...
    return {
        "status": "ok",
        "service": "gemini2.5-live-pipecat-outbound",
...
def main() -> None:
    ...
    logger.info(
        f"Starting Gemini Live Pipecat Outbound Voice Agent on port {SERVER_PORT}"
    )
    uvicorn.run(
