...
async def test_multiturn_voice():
    ...
    ws_url = (
        ...
        "eyJjYWxsX3V1aWQiOiAidGVzdCIsICJmcm9tIjogIisxNTU1MTIzNDU2NyIs"
        "ICJ0byI6ICIrMTY1NzIzMzg4OTIifQ=="
    )
    ...
    try:
        async with websockets.connect(ws_url, close_timeout=5) as ws:
            ...
            print("SUMMARY")
            print("=" * 70)
            header = f"{'Turn':<5} {'Description':<32} {'Sent':>10} "
            header += f"{'Recv':>10} {'TTFR':>8}"
            print(header)
            print("-" * 70)
            ...
            for i, (desc, sent, recv, ttfr) in enumerate(results):
                ...
                print(
                    f"{i:<5} {desc:<32} {sent:>9}B "
                    f"{recv:>9}B {ttfr_str:>8}"
                )
                total_recv += recv
