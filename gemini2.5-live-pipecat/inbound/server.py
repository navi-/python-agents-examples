...
app = FastAPI(
    title="Gemini Live Pipecat Voice Agent (Inbound)",
    description="Inbound voice agent using Pipecat with Google Gemini Live API",
    version="0.1.0",
)
...
async def health_check() -> dict:
    ...
    from inbound.agent import GEMINI_MODEL

    phone = normalize_phone_number(PLIVO_PHONE_NUMBER)
    return {
        "status": "ok",
        "service": "gemini2.5-live-pipecat",
        "model": GEMINI_MODEL,
...
def main() -> None:
    ...
    logger.info(f"Starting Gemini Live Pipecat Inbound Voice Agent on port {SERVER_PORT}")
