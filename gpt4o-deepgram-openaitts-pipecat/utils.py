...
Note: VAD is handled by Pipecat framework (vad_enabled=True in transport params).
"""
...
OPENAI_TTS_SAMPLE_RATE = 24000  # OpenAI TTS default PCM sample rate
DEEPGRAM_SAMPLE_RATE = 8000  # Deepgram receives μ-law at 8kHz directly
...
def plivo_to_openai(mulaw_8k: bytes) -> bytes:
    """Convert Plivo audio (μ-law 8kHz) to OpenAI format (PCM16 24kHz)."""
    pcm_8k = ulaw_to_pcm(mulaw_8k)
    return resample_audio(pcm_8k, PLIVO_SAMPLE_RATE, OPENAI_TTS_SAMPLE_RATE)


def openai_to_plivo(pcm_24k: bytes) -> bytes:
    """Convert OpenAI audio (PCM16 24kHz) to Plivo format (μ-law 8kHz)."""
    pcm_8k = resample_audio(pcm_24k, OPENAI_TTS_SAMPLE_RATE, PLIVO_SAMPLE_RATE)
