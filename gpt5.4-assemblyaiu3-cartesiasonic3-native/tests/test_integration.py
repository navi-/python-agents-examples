"""
Integration tests for GPT-AssemblyAI-Cartesia Voice Agent.
...
3. AssemblyAI Integration - Test AssemblyAI Streaming API connection
...
class TestUnitAudioConversion:
    ...
    def test_plivo_to_assemblyai_conversion(self):
        """Test Plivo to AssemblyAI audio format conversion."""
        from utils import plivo_to_assemblyai

        # 160 bytes μ-law at 8kHz = 20ms
        ulaw_data = b"\xff" * 160
        pcm_16k = plivo_to_assemblyai(ulaw_data)

        # Should be resampled from 8kHz to 16kHz: 2x samples, 2 bytes each
        expected_len = 160 * 2 * 2  # 160 samples * 2x resample * 2 bytes
        assert len(pcm_16k) == expected_len

    def test_cartesia_to_plivo_conversion(self):
        """Test Cartesia to Plivo audio format conversion."""
        from utils import cartesia_to_plivo

        # 480 samples at 24kHz = 20ms, 2 bytes each
        pcm_24k = b"\x00" * 960
        ulaw_8k = cartesia_to_plivo(pcm_24k)

        # Should be resampled from 24kHz to 8kHz: 1/3 samples
        expected_len = 480 // 3  # 160 samples of μ-law
        assert len(ulaw_8k) == expected_len
...
class TestAssemblyAIIntegration:
    """Integration tests for AssemblyAI Streaming API."""

    @pytest.fixture
    def assemblyai_configured(self):
        """Check if AssemblyAI is configured."""
        if not ASSEMBLYAI_API_KEY:
            pytest.skip("ASSEMBLYAI_API_KEY not configured")

    @pytest.mark.asyncio
    async def test_assemblyai_connection(self, assemblyai_configured):
        """Test basic AssemblyAI Streaming API connection."""
