"""
Integration tests for GPT Realtime-Plivo Voice Agent.
...
3. OpenAI Integration - Test OpenAI GPT Realtime API connection
...
Run specific test level:
    ...
    uv run pytest tests/test_integration.py -v -k "openai"
"""
...
class TestOpenAIIntegration:
    """Integration tests for OpenAI GPT Realtime API."""
    ...
    @pytest.mark.asyncio
    async def test_openai_connection(self, openai_configured):
        """Test basic OpenAI GPT Realtime API connection."""
        import websockets as ws_lib

        url = f"wss://api.openai.com/v1/realtime?model={GPT_REALTIME_MODEL}"
        headers = {"Authorization": f"Bearer {OPENAI_API_KEY}"}

        async with ws_lib.connect(url, additional_headers=headers, max_size=None) as ws:
            # Send GA session config
            await ws.send(
                json.dumps(
                    {
                        "type": "session.update",
            ...
            while True:
                ...
                if msg.get("type") == "error":
                    pytest.fail(f"OpenAI error: {msg}")
            assert msg.get("type") == "session.updated"

    @pytest.mark.asyncio
    async def test_openai_text_to_audio(self, openai_configured):
        """Test sending text and receiving audio from OpenAI."""
        import websockets as ws_lib
        ...
        print(f"Events received: {all_events}")
        assert len(audio_chunks) > 0, (
            f"No audio received from OpenAI. Events: {all_events}"
        )
