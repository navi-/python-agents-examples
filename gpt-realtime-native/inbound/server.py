...
app = FastAPI(
    title="GPT Realtime-Plivo Voice Agent (Inbound)",
    description="Inbound voice agent using OpenAI GPT Realtime with Plivo telephony",
    version="0.1.0",
)
...
def configure_plivo_webhooks() -> bool:
    ...
    try:
        ...
        app_name = "GPTRealtime_Voice_Agent"
...
async def health_check() -> dict:
    ...
    return {
        "status": "ok",
        "service": "gpt-realtime-plivo-voice-agent-inbound",
...
def main() -> None:
    ...
    logger.info(f"Starting GPT Realtime-Plivo Inbound Voice Agent on port {SERVER_PORT}")
