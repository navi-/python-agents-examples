...
app = FastAPI(
    title="GPT-AssemblyAI-Cartesia Voice Agent (Inbound)",
    description="Inbound voice agent using GPT-5.4-mini + AssemblyAI STT + Cartesia TTS with Plivo",
    version="0.1.0",
)
...
def configure_plivo_webhooks() -> bool:
    ...
    try:
        ...
        app_name = "GPT_AssemblyAI_Cartesia_Voice_Agent"
...
async def health_check() -> dict:
    ...
    return {
        "status": "ok",
        "service": "gpt-assemblyai-cartesia-voice-agent-inbound",
...
def main() -> None:
    ...
    logger.info(f"Starting GPT-AssemblyAI-Cartesia Inbound Voice Agent on port {SERVER_PORT}")
