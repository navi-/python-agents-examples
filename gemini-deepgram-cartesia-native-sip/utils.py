...
# Plivo credentials (for outbound call API)
PLIVO_AUTH_ID = os.getenv("PLIVO_AUTH_ID", "")
PLIVO_AUTH_TOKEN = os.getenv("PLIVO_AUTH_TOKEN", "")
PLIVO_PHONE_NUMBER = os.getenv("PLIVO_PHONE_NUMBER", "")
PLIVO_FROM_NUMBER = os.getenv("PLIVO_FROM_NUMBER", "")
PLIVO_SIP_TRUNK_HOST = os.getenv("PLIVO_SIP_TRUNK_HOST", "sip.plivo.com")
DEFAULT_COUNTRY_CODE = os.getenv("DEFAULT_COUNTRY_CODE", "US")

# Public URL for webhooks (ngrok URL or production domain)
PUBLIC_URL = os.getenv("PUBLIC_URL", "")

# Public IP for SIP/RTP (used in SDP and IP ACL whitelisting)
# If not set, auto-detected at runtime via external service
...
PLIVO_SAMPLE_RATE = 8000  # SIP/RTP uses 8kHz G.711
DEEPGRAM_SAMPLE_RATE = 8000  # Deepgram accepts 8kHz
...
def cartesia_to_plivo(pcm_24k: bytes) -> bytes:
    """Convert Cartesia audio (PCM16 24kHz) to RTP format (PCM16 8kHz)."""
    return resample_audio(pcm_24k, CARTESIA_SAMPLE_RATE, PLIVO_SAMPLE_RATE)


def pcm8k_to_vad(pcm_8k: bytes) -> np.ndarray:
    """Convert PCM16 8kHz audio to Silero VAD format (float32 16kHz)."""
