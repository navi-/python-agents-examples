...
pytestmark = pytest.mark.skipif(
    not all(
        [PLIVO_AUTH_ID, PLIVO_AUTH_TOKEN, PLIVO_PHONE_NUMBER, PLIVO_TEST_NUMBER, OPENAI_API_KEY]
    ),
    reason="Plivo credentials, PLIVO_TEST_NUMBER, or OPENAI_API_KEY not configured",
)
...
def bleg_app_id(plivo_client, ngrok_tunnel):
    ...
    app_name = "GPTRealtime_Outbound_Test_Agent"
...
class TestOutboundCall:
    ...
    def test_outbound_call_status_lifecycle(
        ...
        status_resp = httpx.get(
            f"{public_url}/outbound/status/{call_id}", timeout=10.0,
        )
