"""Standalone FastAPI server for outbound calls with LiveKit agent worker."""
...
app = FastAPI(
    title="GPT-5.4-mini AssemblyAI Cartesia LiveKit Voice Agent (Outbound)",
    description=(
        "Outbound voice agent using LiveKit with AssemblyAI STT, "
...
async def health_check() -> dict:
    ...
    return {
        "status": "ok",
        "service": "gpt5.4mini-assemblyai-cartesia-livekit-outbound",
...
async def outbound_initiate(
    ...
    Creates a call record, creates a LiveKit room, then uses the Plivo API
    to place a call. When the callee answers, Plivo routes audio to LiveKit
    via SIP, where the voice agent handles the conversation.
    """
    if not phone_number:
    ...
    try:
        # Create LiveKit room for this outbound call
        room_name = await create_outbound_room(record.call_id)
        if room_name:
            call_manager.update_status(record.call_id, record.status, livekit_room=room_name)
            store_room_context(room_name, record.system_prompt, record.initial_message)
...
async def outbound_answer_webhook(
    ...
    Routes the call to the LiveKit room via SIP.
    """
    call_uuid = CallUUID
    from_number = From
    to_number = To
    ...
    record = call_manager.get_call(call_id) if call_id else None
    room_name = record.livekit_room if record else f"outbound-{call_id}"

    if LIVEKIT_SIP_URI:
        sip_uri = f"sip:{room_name}@{LIVEKIT_SIP_URI}"
        logger.info(f"Routing outbound call to LiveKit SIP: {sip_uri}")
    ...
    # Fallback
    logger.warning("LiveKit SIP not configured for outbound call")
    response = plivoxml.ResponseElement()
...
def main() -> None:
    """Run the outbound server with LiveKit agent worker."""
    logger.info(
        f"Starting GPT-5.4-mini AssemblyAI Cartesia LiveKit "
        f"Outbound Voice Agent on port {SERVER_PORT}"
    )

    # Start LiveKit agent worker in background
    if LIVEKIT_URL:
        logger.info("Starting LiveKit agent worker for outbound...")
        worker_thread = threading.Thread(target=_run_livekit_worker, daemon=True)
        worker_thread.start()
    else:
        logger.warning("LIVEKIT_URL not configured. LiveKit agent worker not started.")
