...
app = FastAPI(
    title="Gemini-Deepgram-ElevenLabs Voice Agent (Outbound)",
    description="Outbound voice agent using Gemini, Deepgram, ElevenLabs",
    version="0.1.0",
)
...
async def health_check() -> dict:
    ...
    return {
        "status": "ok",
        "service": "gemini2-deepgramnova2-elevenflashv2.5-voice-agent-outbound",
...
async def outbound_initiate(
    ...
    if not all([PLIVO_AUTH_ID, PLIVO_AUTH_TOKEN, PLIVO_PHONE_NUMBER]):
        return {
            "error": "Plivo credentials or PLIVO_PHONE_NUMBER not configured"
        }
    ...
    try:
        ...
        answer_url = (
            f"{PUBLIC_URL}/outbound/answer?call_id={record.call_id}"
        )
        ...
        call_manager.update_status(
            record.call_id, "failed", outcome="failed"
        )
    ...
    try:
        ...
        for record in call_manager.get_active_calls():
            if (
                ...
                logger.info(
                    f"Outbound call {record.call_id} completed: "
                    f"outcome={outcome}"
                )
                break
    except Exception as e:
...
async def outbound_hangup_call(call_id: str) -> dict:
    ...
    if not record.plivo_call_uuid:
        return {
            "error": "No Plivo call UUID -- call may not be connected yet"
        }
...
async def hold_webhook() -> Response:
    """Hold endpoint - keeps call alive silently."""
    response = plivoxml.ResponseElement()
...
def main() -> None:
    ...
    logger.info(
        f"Starting Gemini-Deepgram-ElevenLabs Outbound Voice Agent "
