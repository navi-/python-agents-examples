...
app = FastAPI(
    title="Gemini-Deepgram-ElevenLabs Voice Agent (Inbound)",
    description="Inbound voice agent using Gemini, Deepgram, ElevenLabs",
    version="0.1.0",
)
...
def configure_plivo_webhooks() -> bool:
    ...
    if not all([PLIVO_AUTH_ID, PLIVO_AUTH_TOKEN, PLIVO_PHONE_NUMBER, PUBLIC_URL]):
        ...
        logger.warning(
            f"Skipping Plivo auto-config. Missing: {', '.join(missing)}"
        )
        return False
    ...
    try:
        ...
        if not phone_number:
            logger.error(
                f"Invalid phone number format: {PLIVO_PHONE_NUMBER}"
            )
            return False
...
async def health_check() -> dict:
    ...
    return {
        "status": "ok",
        "service": "gemini2-deepgramnova2-elevenflashv2.5-voice-agent-inbound",
...
async def answer_webhook(
    ...
    logger.info(
        f"Incoming call: CallUUID={call_uuid}, "
...
def main() -> None:
    ...
    logger.info(
        f"Starting Gemini-Deepgram-ElevenLabs Inbound Voice Agent "
    ...
    if PLIVO_PHONE_NUMBER and PUBLIC_URL:
        ...
        if configure_plivo_webhooks():
            ...
            logger.warning(
                "Plivo auto-configuration failed. Configure manually."
            )
    else:
        logger.info(
            "To enable auto-configuration, "
            "set PUBLIC_URL and PLIVO_PHONE_NUMBER"
        )
