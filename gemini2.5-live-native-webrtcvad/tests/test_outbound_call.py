...
Usage:
    cd gemini2.5-live-native-webrtcvad
...
class TestOutboundCall:
    ...
    def test_outbound_call_full_cycle(
        ...
        if not call_uuid:
            print("[Outbound] Call did not go live — skipping recording")
        ...
        try:
            ...
            try:
                ...
                print(f"[Outbound] Hangup error: {e}")
    ...
    def test_outbound_call_status_lifecycle(
        self, server_process, ngrok_tunnel, bleg_app_id
    ):
        ...
        resp = httpx.post(
            ...
            params={
                "phone_number": PLIVO_TEST_NUMBER,
                "opening_reason": (
                    "your recent free trial sign-up for TechFlow"
                ),
            },
            timeout=30.0,
        )
        ...
        if status["status"] == "failed":
            print("[Lifecycle] Call failed to initiate — cannot test lifecycle")
            return
    ...
    def test_outbound_campaign_endpoint(self, server_process, ngrok_tunnel):
        ...
        time.sleep(3)
        httpx.post(
            f"{public_url}/outbound/hangup/{call_id}", timeout=10.0
        )
